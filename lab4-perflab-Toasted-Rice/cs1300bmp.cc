...
cs1300bmp_readfile(char *filename, struct cs1300bmp *image)
{
  ...
  if ( error ) {
    ...
    image -> width = width;
    image -> height = height;
	image -> color[COLOR_RED  ] = rarray;
	image -> color[COLOR_GREEN] = garray;
	image -> color[COLOR_BLUE ] = barray;
...
//   int colorbytes = image -> width * image -> height;
  ...
  unsigned char *rarray = image -> color[COLOR_RED  ];
  unsigned char *garray = image -> color[COLOR_GREEN];
  unsigned char *barray = image -> color[COLOR_BLUE ];
